...
async function getApiHeaders(env) {
    ...
    if (!token) {
        const tokenResult = await createApiToken(
            ...
            // 授予 'admin'：本 fork 采用 fail-closed 权限模型，rename/move 等管理端点要求 'admin'。
            // 该 token 仅由服务端 dav 处理器内部使用，不会下发给 WebDAV 客户端（客户端用独立的用户名/密码鉴权）。
            ['list', 'upload', 'delete', 'admin'],
            'system',
            null,
            false,
            'internal'
        ...
        // 自愈：确保已存在的 token 具有 'admin' 权限，以便执行 MOVE/rename 操作
        ...
        if (settings.apiTokens?.tokens?.[tokenId]) {
            ...
            if (!tokenData.permissions.includes('admin')) {
                tokenData.permissions.push('admin');
