...
export async function getPageConfig(db, env) {
    ...
    config.push(
        ...
        },
        {
            ...
            type: 'boolean',
            default: false,
            tooltip: '控制上传页面是否展示已有目录列表，默认关闭；关闭后上传页面无法查看已存在的文件夹，仅可手动输入目录',
            tooltip_en: 'Show existing directory list on upload page. Off by default; when off, folders are hidden and paths must be typed manually',
            category: '客户端设置',
            category_en: 'Client Settings',
        },
        {
